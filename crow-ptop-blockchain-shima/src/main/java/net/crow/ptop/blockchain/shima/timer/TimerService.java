...
import net.crow.ptop.blockchain.shima.service.SynchronizeRemoteNodeBlockService;
...
public class TimerService {
    ...
    private void addSeedNodeToLocalBlockchain() {
        for(String strNode:GlobalConfig.SEED_NODE_LIST){
            Node node = new Node();
    ...
    public void searchNewNodes() {
        ...
        for(Node node:nodes){
            ...
            if(isPingSuccess){
                ...
                //链ID不同
                if(!GlobalConfig.BLOCK_CHAIN_ID.equals(pingResponse.getBlockChainId())){
    ...
    private void broadcastLocalBlcokChainHeight() throws Exception {
        ...
        /**
         * 性能调整 根据网络带宽设置传播宽度，这里存在可能你所发送的节点一起向你请求数据。
         */
        //通知按照区块高度较高的先
